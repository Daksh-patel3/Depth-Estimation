#!/bin/bash -l
#SBATCH --job-name=mde_h100_wenlong
...
# ---------------------------------------------------------------------------
# MDE Viewpoint Benchmark — H100 / Wenlong account
# ---------------------------------------------------------------------------
...
# 2. Redirect data / cache / results to scratch (keeps /project quota free)
...
# 3. Make HF / torch.hub download into scratch instead of $HOME
...
# 4. Wire datasets, results, checkpoints to scratch.
#    datasets/ is a real directory with per-dataset symlinks inside it;
#    do NOT replace it with a top-level symlink (breaks the splits dir).
...
# results + checkpoints can be full symlinks to scratch
...
# 5. (Optional) Authenticate Weights & Biases — currently unused by the
#    benchmark scripts, but here for parity with your nanochat job.
# export WANDB_API_KEY="..."
# wandb login --relogin "$WANDB_API_KEY"

# 6. Run the pipeline. Comment out steps you don't need on this submission.
